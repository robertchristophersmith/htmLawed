<?php
/**
 * htmLawed 1.2.0-BETA6, 2 August 2014
 *
 * "A highly customizable PHP script to sanitize / make (X)HTML secure against XSS attacks,
 * so users can edit HTML without risk of your site getting compromised"
 *
 * NOTE: This particular file version has been modified so that all functions reside
 * in the namespace 'htmLawed' to avoid collisions in the global namespace and to
 * follow general coding best practices. The following example illustrates this mod:

```php
$config = array('valid_xhtml'=>1);
$spec = 'img=vFlag; input=rel';
$newText = \htmLawed\hl('a string of HTML text', $config, $spec);
```

 * @author    Robert C Smith (minor changes as noted above)
 * @author    Santosh Patnaik (original author)
 * @copyright 2014 Santosh Patnaik
 * @license   http://www.gnu.org/licenses/lgpl-3.0.txt LGPL 3
 * @license   http://www.gnu.org/licenses/gpl-2.0.txt GPL 2+
 * @see       htmLawed_README.txt text version of README file
 * @see       htmLawed_README.htm HTML version of README file
 * @see       http://www.bioinformatics.org/phplabware/internal_utilities/htmLawed/beta for more detailed information
 */
namespace htmLawed {
    /**
     * @param       $t
    ...
    function hl($t, $C = 1, $S = array())
    {
        ...
        $C['deny_attribute'] = $x;
        // config URL
        $x = (isset($C['schemes'][2]) && strpos($C['schemes'], ':')) ? strtolower($C['schemes']) :
        ...
        $C['anti_link_spam']     = (isset($C['anti_link_spam']) && is_array($C['anti_link_spam']) && count($C['anti_link_spam']) == 2 && (empty($C['anti_link_spam'][0]) or hl_regex(
                    $C['anti_link_spam'][0]
                )) && (empty($C['anti_link_spam'][1]) or hl_regex($C['anti_link_spam'][1]))) ? $C['anti_link_spam'] : 0;
    ...
    {
        // by content
        $cB = array('blockquote' => 1, 'form' => 1, 'map' => 1, 'noscript' => 1);
        // Block
        $cE = array(
            'area'    => 1,
            'br'      => 1,
    ...
    function hl_ent($t)
    {
        ...
        if ($t[0] != '#') {
            return ($C['and_mark'] ? "\x06" : '&') . (isset($U[$t]) ? $t : (isset($N[$t]) ? (!$C['named_entity'] ? '#' . ($C['hexdec_entity'] > 1 ? 'x' . dechex(
                        $N[$t]
                    ) : $N[$t]) : $t) : 'amp;' . $t)) . ';';
        }
        if (($n = ctype_digit($t = substr($t, 1)) ? intval($t) : hexdec(
                substr($t, 1)
            )) < 9 or ($n > 13 && $n < 32) or $n == 11 or $n == 12 or ($n > 126 && $n < 160 && $n != 133) or ($n > 55295 && ($n < 57344 or ($n > 64975 && $n < 64992) or $n == 65534 or $n == 65535 or $n > 1114111))
        ) {
...
// eof
    ...
    function hl_tag($t)
    {
        ...
        $nfr = 0;
        foreach ($aA as $k => $v) {
            if (((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) && (isset($aN[$k][$e]) or isset($aNU[$k]) or preg_match(
                            '`data-((?!xml)[^:]+$)`',
                            $k
                        )) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])
            ) {
                if (isset($aNE[$k])) {
                    $v = $k;
    ...
    /**
     * Return the semantic version of HTMLawed used in this file
     * @return string
     */
    function hl_version()
    {
